...
bool xr_file_system::parse_fs_spec(xr_reader& r)
{
	...
	for (unsigned line = 1; p < end; p = next_line(p, end), ++line) {
		...
		if (c == '$') {
			...
			if (i < 2)
				values[1].clear();
			auto* parent_pa = find_path_alias(values[0].c_str());
			path_alias* pa = add_path_alias(alias, parent_pa ? parent_pa->path : values[0], values[1]);
			if (pa == 0) {
				msg("can't parse line %u", line);
				return false;
			}
...
xr_reader* xr_file_system::r_open(const char* path, const char* name) const
{
	...
	return r_open(std::string(path) + "\\" + name);
}
...
xr_writer* xr_file_system::w_open(const char* path, const char* name) const
{
	const path_alias* pa = find_path_alias(path);
	if (pa != nullptr) {
		return w_open(pa->root + name);
	}
	return w_open(std::string(path) + "\\" + name);
}
...
bool xr_file_system::folder_exist(const char* path, const char* name) const
{
	const path_alias* pa = find_path_alias(path);
	if (pa != nullptr) {
		return folder_exist(pa->root + name);
	}
	return folder_exist(std::string(path) + "\\" + name);
}
...
bool xr_file_system::file_exist(const char* path, const char* name) const
{
	const path_alias* pa = find_path_alias(path);
	if (pa != nullptr) {
		return file_exist(pa->root + name);
	}
	return file_exist(std::string(path) + "\\" + name);
}
...
uint32_t xr_file_system::file_age(const char* path, const char* name) const
{
	const path_alias* pa = find_path_alias(path);
	if (pa != nullptr) {
		return file_age(pa->root + name);
	}
	return file_age(std::string(path) + "\\" + name);
}
