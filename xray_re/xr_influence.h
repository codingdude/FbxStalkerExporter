...
using fbone_weight = _bone_weight<uint32_t, float>;

template<typename Tb, typename Tw>
using _bone_weights = _svector<_bone_weight<Tb, Tw>, 4>;

// up to 2 influences in SoC, and up to 4 in CS.
template<typename Tb, typename Tw> struct _influence: public _bone_weights<Tb, Tw> {
...
using finfluence = _influence<uint32_t, float>;
...
{
	_bone_weights<Tb, Tw>::push_back(_bone_weight<Tb, Tw>(bone0, Tw(1)));
}
...
#endif
		if (bw0 < bw1) {
			_bone_weights<Tb, Tw>::push_back(bw0);
			_bone_weights<Tb, Tw>::push_back(bw1);
		} else {
			_bone_weights<Tb, Tw>::push_back(bw1);
			_bone_weights<Tb, Tw>::push_back(bw0);
		}
...
void _influence<Tb, Tw>::set(size_t num_bones, const uint16_t* bones, const float* weights)
{
	for (float sum = 0, w; num_bones;) {
		...
		_bone_weights<Tb, Tw>::push_back(bw);
skip:;
	}
	reorder();
}
