...
void xr_object::load_object(xr_reader& r)
{
	...
	if (r.find_chunk(EOBJ_CHUNK_SURFACES_2)) {
		r.r_seq(r.r_u32(), m_surfaces, &xr_surface::load_2);
		r.debug_find_chunk();
	} else if (r.find_chunk(EOBJ_CHUNK_SURFACES_1)) {
		r.r_seq(r.r_u32(), m_surfaces, &xr_surface::load_1);
		r.debug_find_chunk();
	} else if (r.find_chunk(EOBJ_CHUNK_SURFACES_0)) {
		r.r_seq(r.r_u32(), m_surfaces, &xr_surface::load_0);
		r.debug_find_chunk();
	...
	if (s) {
		s->r_chunks(m_bones, &xr_bone::load_1);
		r.close_chunk(s);
	} else if (r.find_chunk(EOBJ_CHUNK_BONES_0)) {
		s->r_seq(r.r_u32(), m_bones, &xr_bone::load_0);
		r.debug_find_chunk();
	}
	setup_bones();
	if (r.find_chunk(EOBJ_CHUNK_MOTIONS)) {
		r.r_seq(r.r_u32(), m_motions, &xr_skl_motion::load);
		r.debug_find_chunk();
	}
	...
	if (r.find_chunk(EOBJ_CHUNK_PARTITIONS_0)) {
		...
		r.r_seq(r.r_u32(), m_partitions, &xr_partition::load_1);
		r.debug_find_chunk();
	}
...
void xr_object::save_object(xr_writer& w) const
{
	...
	if (!m_partitions.empty()) {
		...
		w.w_seq(m_partitions, &xr_partition::save);
		w.close_chunk();
	}
...
{
	r.r_chunks(m_bones, &xr_bone::load_data);
...
{
	w.w_chunks(m_bones, &xr_bone::save_data);
	w.open_chunk(EOBJ_CHUNK_PARTITIONS_1);
	w.w_size_u32(m_partitions.size());
	w.w_seq(m_partitions, &xr_partition::save);
	w.close_chunk();
