...
{
	char level[1024];
	std::snprintf(level, sizeof(level), "%s\\level", level_path);
	xr_file_system& fs = xr_file_system::instance();
	xr_reader* r = fs.r_open(game_data_path, level);
	if (r == 0)
		return false;
	bool status = false;
	if (r->find_chunk(FSL_HEADER)) {
		...
		case XRLC_VERSION_14:
			...
			break;
		default:
			msg("unknown xrLC version %u", xrlc_version);
			break;
		}
	}
	fs.r_close(r);
...
void xr_level::load(uint32_t xrlc_version, const char* game_data_path, const char* level_path, xr_reader& r)
{
	...
	xr_file_system& fs = xr_file_system::instance();
	if (!fs.resolve_path(game_data_path, level_path, full_path)) {
		msg("Can't resolve level path");
		return;
	}
	level_path = full_path.c_str();
